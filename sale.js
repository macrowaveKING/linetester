    ...
    webix.ui({
      ...
      on:{
        onItemClick:function(id){
          ...
          var saleId=list.getItem(listId).saleId;
          ...
          global.getConnection().query("update sale set payStatus=? where saleId=?",[id,saleId],(error,results,fields)=>{
            if (error) {
              throw error;
            }
...
//绑定事件
  ...
  attachEvents(){
    ...
    $$("deleteBtn").attachEvent("onItemClick",()=>{
      var source=$$("detailSale").serialize();
      if (source.length<=0) {
        return;
      }
      console.log(source);
    ...
    $$("more").attachEvent("onItemClick",()=>{
      this.updateSales();
      $$("detailSale").showItemByIndex(this.current)
    })
    ...
    $$("eraserBtn").attachEvent("onItemClick",()=>{
      ...
      var sale=$$("detailSale").getSelectedItem()
      ...
      this.connection.beginTransaction((error)=>{
        ...
        //delete query begin
        this.connection.query("delete from sale where saleId=?",sale.saleId,(error,results,fields)=>{
          if(error){
            return this.connection.rollback((error)=>{
              throw error;
            })
          }
          this.connection.query("update storage set quantity=quantity-?,lastUpdate=? where productId=?",[sale.quantity,new Date(),sale.productId],(error,results,fields)=>{
            if (error) {
              return this.connection.rollback((error)=>{
                throw error;
              })
    ...
    $$("detailSale").attachEvent("onItemDblClick",(id,evt,node)=>{
      var sale=$$("detailSale").getItem(id);
    ...
    $$("purchaseBtn").attachEvent("onItemClick",()=>{
      ...
      this.connection.beginTransaction((error)=>{
        ...
        this.connection.query("insert into sale set ?",sale,(error,results,fields)=>{
          console.log("insert into sale error:"+error);
          if (error) {
            return this.connection.rollback((error)=>{
              if (error) {
                throw error;
          ...
          newSale=sale;
          newSale.saleId=results.insertId;
          newSale.id=results.InsertId;
          console.log("sale results:");
          console.log(results);
                ...
                this.connection.commit((error)=>{
                  ...
                  if (error) {
                    ...
                    for(var key in user){
                      ...
                      newSale[key]=user[key];
                    }
                    for (var key in product) {
                      newSale[key]=product[key];
                    }
                    //newSale.index=$$("detailSale").count();
                    delete newSale["id"];
                    $$("detailSale").add(newSale);
  ...
  updateSales(){
    ...
    var queryStatement='select groupNo,unit,productName,purchaseDate, saleId,quantity,price,userName,email,sale.userId,userName,chinese,brand,sale.productId,specification,productType,payStatus,position,phone,fix,company ';
    var countStatement='select count(*) as total '
    var tables=' from Sale,Users,Product where sale.productId=product.productId and sale.userId=users.userId '
